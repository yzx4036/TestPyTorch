...
class DeepQNetwork(nn.Module):
    def __init__(self, lr, n_actions, name, input_dims, fc1_dims, fc2_dims1):
        super(DeepQNetwork, self).__init__()
        self.name = name
        self.checkpoint_file = os.path.join("../models/", name)
        
        self.fc1_dims = fc1_dims * len(input_dims)  # number of neurons in first hidden layer 第一个隐藏层的神经元个数
        self.fc2_dims1 = fc2_dims1 * len(input_dims)  # number of neurons in second hidden layer 第二个隐藏层的神经元个数
        ...
        self.fc3 = nn.Linear(self.fc2_dims1, n_actions)  # 创建第三个全连接层，输入维度为fc2_dims，输出维度为n_actions
